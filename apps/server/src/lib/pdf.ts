...
import { getCloudinaryDownloadUrl } from "./cloudinary.js";
...
export async function extractPdfFromBuffer(buffer: ArrayBuffer | Buffer): Promise<PdfExtractResult> {
    const arrayBuffer = buffer instanceof Buffer ? (buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength) as ArrayBuffer) : buffer;

    const pdf = await getDocumentProxy(new Uint8Array(arrayBuffer));
    const { totalPages, text } = await extractText(pdf, { mergePages: false });

    const pages = Array.isArray(text) ? text.map((page) => page.trim()) : [String(text).trim()];
...
export async function extractPdfFromCloudinary(input: {
    ...
    try {
        ...
        const signedUrl = getCloudinaryDownloadUrl(
            input.publicId,
