...
export function getCloudinaryDownloadUrl(publicId: string, resourceType: "raw" | "image" = "raw") {
    if (!cloudName || !uploadPreset || !apiKey || !apiSecret || !cloudUrl) {
...
export async function uploadPdfToCloudinary(buffer: Buffer, filename: string): Promise<CloudinaryUploadResult> {
    if (!cloudName) {
        throw new ValidationError("Cloudinary is not configured on the server");
    }

    if (!uploadPreset) {
        throw new ValidationError("CLOUDINARY_UPLOAD_PRESET is required for PDF uploads");
    }
